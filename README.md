...
## Resum
El dataset que s'utilitza en aquesta pràctica és la informació obtinguda d'una enquesta de satisfacció pels passatgers d'una aerolínia.
En el Kaggle se'ns faciliten dues bases de dades, una per fer el train i l'altra per fer el test. Totes dues tenen els mateixos atributs, però tenen dades diferents.
La base de dades 'train.csv' consta de 103904 dades amb 25 atributs, mentre que la base de dades 'test.csv' té 25976 dades i 25 atributs.
Els atributs que apareixen als dos datasets són:
  - 'Unnamed: 0': identificació de l'enquesta.
  - 'id': identificació del passatger.
  ...
  - 'Arrival Delay in Minutes': minuts de retard de l'arribada del vol.
...
Com podem veure, tenim molts atributs de tipus objecte, cosa que no m'interessa, així que el que he fet és passar-los a números. Per exemple, en el cas de l'atribut 'Gender', la resposta 'female' passarà a ser un 0, i la resposta 'male' passarà a ser un 1, i així amb la resta d'atributs de tipus objecte.
...
Llavors el que comencem a fer és analitzar cada atribut d'un en un per veure com es relacionen amb l'atribut objectiu 'satisfaction'.  Ho farem de diferent manera pels atributs categòrics i els quantitatius.
...
- Els atributs 'Departure Delay in Minutes' i 'Arrival Delay in Minutes' estàn molt correlacionats, i graficant-los veiem que tenen una relació linial.
...
Amb el model XGBoost hem fet un estudi dels atributs que tenen més importància quan un passatger pren la  decisió de donar una valoració positiva o negativa de l'aerolínia, i hem vist que aquests són,per ordre d'importància:
...
## Demo
Per tal de fer una prova, es pot omplir i executar el fitxer "Demo.py".

## Conclusions
Hem vist que el model que millor prediu el nostre atribut objectiu és el XGBoost.
A més , hem observat que si volem que els passatgers de l'aerolínia la valorin bé, hauríem d'intentar que la sortida i l'arribada dels vols fos puntual, i millorar la neteja i el servei a bord en els vols.
